...
- **Run performance tests** (opt-in, real Postgres, genuinely slow): `uv run pytest tests/performance -m performance` — validates the v1 acceptance criterion that a million-message channel backfills unattended (resumable across a simulated crash) and browses at server-side page-load times under 200ms (`ROADMAP.md`). No live Discord connection needed: `tests/performance/synthetic.py` generates the synthetic messages. `test_million_message_backfill.py` drives the real `backfill_channel()` pipeline at a reduced 100,000-message scale (~1-2 minutes on a healthy local Postgres) rather than the literal 1,000,000 — the checkpoint/resume logic under test doesn't depend on scale, and three separate attempts at the full 1,000,000 each hit a multi-hour, repeatedly-interrupted run in practice; see that test's own docstring and `RESOLVED_ISSUES.md` for the full account. `test_browse_performance.py` bulk-inserts the real, literal 1,000,000 rows directly via one set-based `INSERT ... SELECT generate_series(...)` (seeds in well under a minute) — that test's cost is Postgres query performance, not this pipeline's per-message round-trip count, so it isn't affected by the same scale-down. All four of its cases pass consistently as of the `get_messages_page`/`search_messages` nearest-end fixes (`RESOLVED_ISSUES.md`) — the last page of the board and a deep search both dropped from seconds to well under 100ms. The board index (`/`) itself has been observed landing right at/just over the 200ms line in some runs (99-213ms across three clean runs) — an unexplained, pre-existing flake risk unrelated to either fix (`DESIGN.md` §10), not something to chase if you see it once. **Note for this environment specifically**: a plain `DELETE FROM messages WHERE channel_id = ...` against a million-row, GIN-indexed table was observed taking 5+ minutes (sometimes never finishing) even fully uncontended, apparently a local Docker-Desktop-on-macOS disk I/O characteristic — `TRUNCATE` was consistently near-instant (<1s) for the same cleanup. If a performance-test run seems to hang, it's most likely sitting in exactly this kind of teardown `DELETE`, not stuck logic; a real Postgres deployment may not share this characteristic. Excluded from a plain `uv run pytest` via `addopts`, same as `live_discord`.
...
- **Run the web app**: `uv run threadbare-web` — dev server on `http://localhost:5000` (on macOS, AirPlay Receiver squats on port 5000 by default; disable it in System Settings → General → AirDrop & Handoff, or the request just silently 403s from `ControlCenter`, not Flask). Every page now sits behind the Discord OAuth login gate (§6) — any member of `DISCORD_GUILD_ID` may log in; only members with Manage Server/Administrator on that guild can reach `/admin/`. Needs `DATABASE_URL` pointing at a database with real mirrored content to show anything on the board index.
