...
async def test_apply_migration_creates_table_and_records_version(db_conn):
    await ensure_schema_migrations_table(db_conn)
    migration = _fixture_migration(
        "9999_integration_test_fixture",
        "CREATE TABLE integration_test_fixture_table (id int)",
    )

    await apply_migration(db_conn, migration)

    applied = await applied_migrations(db_conn)
