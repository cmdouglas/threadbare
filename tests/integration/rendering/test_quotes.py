...
async def _seed_message(
    conn, *, message_id, channel_id, author_id, content="hello", reply_to_id=None, posted_at=None
):
    await conn.execute(
        """
        ...
        """,
        (message_id, channel_id, author_id, content, reply_to_id, posted_at),
    )
...
async def _seed_thread_message(
    conn, *, message_id, thread_id, author_id, content="hello", reply_to_id=None, posted_at=None
):
    await conn.execute(
        """
        ...
        VALUES (%s, %s, %s, %s, %s, COALESCE(%s, now()))
        """,
        (message_id, thread_id, author_id, content, reply_to_id, posted_at),
    )
...
async def test_render_reply_quote_for_a_real_reply_chain(db_conn):
    ...
    assert 'href="/board/10/continuous/page/1#post-100"' in html
    assert "alice" in html
    assert "original" in html


async def test_render_reply_quote_href_for_a_reply_inside_a_thread(db_conn):
    await _seed_guild_and_channel(db_conn)
    await _seed_thread(db_conn, thread_id=3000, parent_channel_id=10)
    await _seed_user(db_conn, user_id=1, display_name="alice")
    await _seed_thread_message(
        db_conn, message_id=100, thread_id=3000, author_id=1, content="original"
    )
    await _seed_thread_message(
        db_conn, message_id=101, thread_id=3000, author_id=1, content="a reply", reply_to_id=100
    )
    message_row = await queries.get_message_for_render(db_conn, 101)

    html = await render_reply_quote(db_conn, message_row)

    assert 'href="/topic/3000/page/1#post-100"' in html


async def test_render_reply_quote_href_reflects_the_targets_page_number(db_conn):
    await _seed_guild_and_channel(db_conn)
    await _seed_user(db_conn, user_id=1, display_name="alice")
    base = datetime(2026, 1, 1, tzinfo=UTC)
    # 25 earlier messages (a full first page), so the reply target (the
    # 26th) lands on page 2.
    for i in range(25):
        await _seed_message(
            db_conn,
            message_id=1000 + i,
    ...
    await _seed_message(
        ...
        author_id=1,
        content="original",
        posted_at=base + timedelta(seconds=25),
    )
    await _seed_message(
        db_conn,
        message_id=2001,
        ...
        content="a reply",
        reply_to_id=2000,
        posted_at=base + timedelta(seconds=26),
    )
    message_row = await queries.get_message_for_render(db_conn, 2001)

    html = await render_reply_quote(db_conn, message_row)

    assert 'href="/board/10/continuous/page/2#post-2000"' in html
