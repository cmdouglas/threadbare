...
- [ ] Voice (and stage) channels should never be discoverable or toggleable — already a stated non-goal (`DESIGN.md` §2) but nothing in code enforces it yet. `sync_worker/discovery.py`'s channel-splitting logic, `backfill.py`'s `SKIPPED_CHANNEL_TYPES`, `web/views/wizard.py`'s channel-row filter, and `db/admin_queries.get_channels_with_sync_state`'s `WHERE` clause all only special-case categories today, so a voice channel currently shows up as an ordinary backfillable, admin-toggleable channel. Needs a `VOICE`/`STAGE_VOICE` constant in `channel_types.py` and a filter applied consistently everywhere (or one shared excluded-types set instead of four separate `!= CATEGORY` checks). Whether a voice channel's text-chat sub-feature should ever be indexed if it turns out to hold real content is a separate, open question — see `DESIGN.md` §10.
- [ ] Show each poster's Discord roles and bot/human status. Exact presentation (badge, icon, name color, something else) is undecided — needs a design pass, not just a data change. `users` has no `is_bot` column and no roles table today; the existing role-mention placeholder (§3 above, `rendering/emoji.py`) is the same underlying gap already flagged as out of scope for mentions specifically. Bot status is the cheap half — `discord.Member.bot`/`discord.User.bot` is already available everywhere `transform.user_to_row` runs, just never read; roles are a bigger lift needing a join table plus fetching `member.roles` in the sync worker's discovery and event-handling paths.
- [ ] Admin option to auto-index newly discovered channels by default (permissions allowing), so mods aren't stuck hand-toggling every new channel as it appears. Today `channels.indexed` just defaults `true` at the schema level (migration `0001_initial_schema.sql`) with no way to configure that default, let alone turn it off; would need a persisted site-wide setting (no settings table exists yet — `web/views/admin.py`'s toggle is per-channel only), a read of that setting from `discovery.discover_channels`, and a new admin-page control.
