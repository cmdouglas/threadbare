...
- [x] New `roles` table (id, guild_id, name, permissions, position) and per-channel permission-overwrite tables (role tier + member tier), captured by the sync worker on `GUILD_ROLE_CREATE`/`UPDATE`/`DELETE` and `CHANNEL_UPDATE` — neither is captured anywhere today; `discord_permissions.compute_is_public` and `wizard/preflight.py` both currently read overwrites live off Discord's in-memory objects rather than storing them.
  - The `roles` table itself already existed (migration `0008_user_roles.sql`, for username-color/bot-badge display) but had no `permissions` column — migration `0010_role_permissions_and_overwrites.sql` adds it (`bigint`, since Discord's permission bitfield exceeds 32 bits) plus two new tables, `channel_role_overwrites`/`channel_member_overwrites` (`channel_id`/`role_id` or `user_id`/`allow`/`deny`, both cascade-deleting with their channel/role/user). Overwrites are captured for categories too, not just content channels, since Discord's permission model treats a category as a channel for overwrite purposes.
  - New `sync_worker/channel_overwrites.py::sync_channel_overwrites` reads a live channel/category's `.overwrites` dict and makes the two tables match it exactly (delete-then-bulk-insert, same "make it match exactly" idiom as `sync_message_embeds`/`sync_message_reactions` — a mod can remove an overwrite on Discord, not just add one). Self-heals a `users` row for a member-tier overwrite target who's never posted, same shape as `handle_channel_upsert`'s existing category-parent self-heal. Wired into `discovery.discover_channels` (batch/reconnect) and `events.handle_channel_permissions_changed` (covers both `CHANNEL_UPDATE` and `CHANNEL_CREATE`, the two live events where overwrites can change); deliberately *not* wired into `handle_role_permissions_changed`, since a role's own attribute changing never changes which channels have an overwrite for it.
  - **Real ordering bug found and fixed**: `bot.py`'s `on_ready` called `discover_channels` before `discover_roles` — once `discover_channels` started writing role-tier overwrite rows (which FK-reference `roles.id`), a fresh connect referencing a role not yet discovered would have raised `ForeignKeyViolation` immediately. Fixed by reordering `on_ready` to discover roles first; `discover_channels` doesn't read from the `roles` table itself, so this had no other effect. Regression-tested (`test_discover_roles_before_discover_channels_avoids_an_fk_violation_on_role_overwrites`, plus a unit test asserting `on_ready`'s actual call order).
  - Unit and integration tested (`test_transform.py`, `test_repository_channels.py`, `test_channel_overwrites.py`, `test_discovery.py`, `test_events.py`); no live-Discord verification needed since this only adds storage/capture for data already delivered by events this codebase already subscribes to.
- [x] Persist each member's current role-ID list. `sync_worker/events.handle_member_update` (already shipped for display-name refresh, reusing the `GUILD_MEMBERS` intent — see UI polish backlog above) only reacts to *future* role changes; Phase 2 also needs an initial bulk backfill of every existing member's roles on startup, since that handler alone never populates a member who hasn't changed since the intent was added.
  - New `discovery.discover_member_roles`: fresh REST pagination (`guild.fetch_members(limit=None)`, not the gateway member cache — same "don't trust cache" convention as `discover_channels`/`discover_roles`'s own fresh fetches), upserting each member via the existing `repository.upsert_user`/`transform.user_to_row` (no new repository function needed). Unlike the cheap per-reconnect metadata calls, this is real REST work for a large guild — wired as a new guarded background task in `on_ready` (`_member_role_backfill_task`, matching `_backfill_task`'s existing guard-against-refiring-on-reconnect precedent) rather than run inline on every connect. Commits every 200 members rather than holding one giant transaction open.
  - Unit and integration tested (`test_discovery.py`'s `discover_member_roles` tests — including the exact reported-gap scenario, a member who's never posted; `test_bot.py`'s guard-against-refiring test).
- [x] Generalize Discord's permission-resolution order (base @everyone → role allows/denies → category overwrite → channel overwrite → admin short-circuit) into one shared implementation, rather than a third reimplementation next to the two narrow cases that already exist: `discord_permissions.compute_is_public` (the @everyone-only case) and `wizard/preflight.py`'s `_apply_overwrite_tier`/`compute_bot_effective_permissions` (the bot-only case). Both should end up calling the shared version.
  - New `discord_permissions.compute_effective_permissions()`: base permissions (already OR'd across whatever roles the identity holds — the caller's job) → Administrator short-circuit → category tier → channel tier, each tier resolved via a new `OverwriteTier` (`everyone_overwrite`/`role_overwrites`/`member_overwrite`, three pre-classified slots rather than a single Discord-REST-shaped tagged list) — chosen because the `channel_role_overwrites`/`channel_member_overwrites` tables shipped in the item above are already split into two tables, not one tagged column, so a future per-user-visibility caller builds an `OverwriteTier` from them directly with no re-tagging.
  - `compute_is_public` and `wizard/preflight.compute_bot_effective_permissions` are now both thin wrappers delegating to the shared function, with their existing external signatures completely unchanged — every call site (`discovery.py`, `events.py` via `sync_worker/permissions.py`'s re-export, `web/views/wizard.py`) needed zero edits. `wizard/preflight.py`'s own `_apply_overwrite_tier` was replaced by `_tier_from_rest_overwrites`, an adapter that classifies Discord's REST-JSON tagged overwrite list into an `OverwriteTier` — filtering role overwrites down to roles the identity actually holds happens only here, not inside the shared function, which applies whatever `OverwriteTier` it's given unconditionally (documented contract, regression-tested).
  - Proven behaviorally identical, not just "should be": all 21 pre-existing tests across `test_discord_permissions.py`/`test_preflight.py` pass completely unmodified. New tests cover cases neither narrow wrapper isolated on its own — multiple role overwrites combining (deny-then-allow order), a member overwrite applying with no role tier present, an arbitrary third-party identity's role_ids (proving the function isn't secretly coupled to just "@everyone" or "the bot"), the admin short-circuit called directly, and the caller-filters-first contract.
- [x] Compute a per-user channel-visibility set from stored roles/overwrites at login (and refresh on a timer plus the role/channel-update events above); cache it per session. This replaces `web/authz.py`'s current binary is-a-guild-member gate for channels enrolled in role-gating.
  - **Deviates from the literal wording above, deliberately**: recomputes fresh from Postgres on every call instead of session-caching with invalidation. A cache needs a real invalidation mechanism (a version counter bumped on every role/overwrite/member-role change, compared against a stamp in the session cookie each request) — real complexity in exactly the code `DESIGN.md` §7 calls "the fiddliest... the only place a bug is a disclosure bug, not a rendering bug." Recompute-fresh mirrors `web/app.py`'s existing `resolve_site_title` reasoning (a cheap per-request Postgres read, not cached, so a change shows up immediately) and avoids that whole class of invalidation bug for an unproven perf cost. Revisit caching only if it's shown to be a real bottleneck.
  - New top-level `channel_visibility.py` (pure, dependency-free, DB-shaped sibling of `wizard/preflight.py`'s `_tier_from_rest_overwrites`/`compute_channel_permission_table`): `compute_visible_channel_ids(*, base_permissions, everyone_role_id, channels, role_overwrites, member_overwrites) -> set[int]` classifies pre-fetched `channel_role_overwrites`/`channel_member_overwrites` rows into `OverwriteTier`s per channel (category tier from `parent_id`, channel tier from the channel's own id) and delegates to `discord_permissions.compute_effective_permissions`, same as the wizard's bot-only case does for REST-JSON-shaped overwrites.
  - Four new small `db/queries.py` functions do the actual Postgres reads: `get_base_permissions` (`@everyone`'s permissions OR'd with the user's held roles — `@everyone`'s row id equals the guild's id, Discord's own convention), `get_visibility_channels` (every real content channel + `parent_id`, deliberately unfiltered by `is_public`/`indexed` since a role can regain access `@everyone` can't see), `get_channel_role_overwrites`/`get_channel_member_overwrites` (both `ANY(channel_ids)`-batched across every channel+category in one call each, no N+1 per channel).
  - New `web/authz.py::resolve_visible_channel_ids(conn, *, guild_id, user_id) -> set[int]` orchestrates the four queries plus `channel_visibility`, living in `authz.py` since the ROADMAP bullet itself names this module's binary gate as what it eventually replaces. **Not wired into any view, route, or `before_request` hook yet** — the next bullet below does that; this bullet is scoped to the computation itself, fully tested but currently dead code from the app's perspective.
  - Unit tested (`test_channel_visibility.py`, 7 cases including deny/allow combination order, member-overwrite-with-no-role-tier, administrator short-circuit, and a `parent_id IS NULL` no-category case), integration tested against real Postgres (`test_queries.py`'s four new query functions; new `test_authz.py` re-proving the same scenarios end-to-end through the real DB round trip, including a `user_id` with no `users` row yet falling back to `role_ids=[]` without raising).
- [x] Filter every read path by the requesting user's visibility set, not just global `is_public`/`indexed` — board index, topic/continuous listings, and especially search (today's `db/queries._SEARCH_WHERE_SQL` filters only on `indexed`, with no per-user clause at all) all need a new join/parameter. The easiest bypass vector in the whole phase, per `DESIGN.md` §7's risk note.
- [x] Per-channel opt-in flag on the admin page, separate from the existing `is_public`/`indexed` toggle — mods enroll a role-gated channel into the new visibility system deliberately, never automatically.
  - **Shipped together, deliberately**: filtering can't safely apply to every non-public channel unconditionally (`DESIGN.md`'s upgrade contract says a feature that changes what's exposed must default off and never silently change visibility), so it had to be conditioned on a flag that didn't exist yet — shipping either bullet alone would have been dead code or unsafe. Splitting further (e.g. board/topic this step, search later) was also rejected: a mod who enrolls a channel would reasonably expect it protected everywhere, and partial protection is worse than none in code that's "the only place a bug is a disclosure bug, not a rendering bug."
  - New `channels.visibility_enrolled boolean NOT NULL DEFAULT false` (migration `0011_channel_visibility_enrollment.sql`), mirroring the existing per-channel `indexed` toggle's shape rather than `site_settings`'s guild-wide singleton pattern. A channel with the flag off (the default) behaves in every read path exactly as before upgrade — `is_public`+`indexed` for the board index, `indexed`-only for search/post-history, no check at all for direct board/topic URL navigation (an unchanged, known v1 gap for non-enrolled channels). A channel with the flag on is additionally gated by the requester's real computed permissions (`web/authz.py::resolve_visible_channel_ids`, shipped in the prior bullet): if the requester can't currently see it, it 404s on direct nav and disappears from the index/search/their profile's post history.
  - New `db/queries.py::_visibility_clause()` builds the shared "legacy OR enrolled+visible" SQL fragment (`indexed AND (is_public OR (visibility_enrolled AND id = ANY(visible_channel_ids)))`), reused by `get_boards_and_categories`, `_SEARCH_WHERE_SQL` (and therefore `search_messages`/`count_search_results`), and `get_post_count_for_user`/`get_recent_posts_for_user` — one fragment, four call sites, rather than four independent reimplementations of the same rule. Confirmed directly against Postgres that `id = ANY(%(ids)s)` with an empty list evaluates to false for every row, so an empty `visible_channel_ids` needs no special-casing.
  - New `web/app.py` `before_request` hook (`resolve_visible_channels`, registered after `require_login`) computes `g.visible_channel_ids` fresh once per request via the already-shipped `resolve_visible_channel_ids` — needs its own `is_logged_in()` guard despite running last, since `require_login` only gates on whether login is *required* for the endpoint, not on whether the requester actually is logged in, and an exempt route (`/login` itself) still reaches every hook regardless.
  - New `web/authz.py::channel_passes_visibility_gate(channel, visible_channel_ids)` is the direct-navigation gate, wired into `board.py`'s single `_get_board_or_404` choke point (covering every board route in one change) and `topic.py`'s `topic_page` (which needed a new channel fetch, since `get_thread` has no channel join). The `*_jump*` routes are left unguarded on purpose — they only redirect into a route that itself enforces the gate before rendering anything.
  - New `admin_queries.get_channel_visibility_enrolled`/`set_channel_visibility_enrolled` and `admin.py::toggle_visibility_enrolled`, mirroring `toggle_indexed` exactly; `admin.html` gained a "Visibility-enrolled" column and its own toggle button per channel row.
  - Unit tested (`channel_passes_visibility_gate`'s three cases), integration tested at every layer (`get_boards_and_categories`/`search_messages`/`count_search_results`/`get_post_count_for_user`/`get_recent_posts_for_user`'s three-case matrix — enrolled+visible shown, enrolled+denied hidden, **non-enrolled+role-granted-anyway still hidden**, the critical regression guard proving a role grant alone can never surface a non-enrolled channel; `test_board.py`/`test_topic.py`/`test_board_index.py`/`test_search.py`/`test_user.py`/`test_admin.py`'s new view-level cases), and end-to-end (`tests/e2e/test_admin_and_login_gate.py`'s new full-stack test: a plain member session 404s/sees nothing until a real role grant flips it, proven through a real browser). Manually verified in a real browser too (screenshots): board index, board page, and the new admin toggle all behave correctly before and after a role grant.
...
- [x] Admin option to auto-index newly discovered channels by default (permissions allowing), so mods aren't stuck hand-toggling every new channel as it appears. The live `CHANNEL_CREATE` path already always defaults to `indexed=false` (see above) — this item was about the *other* path: `discovery.discover_channels`'s batch reconnect scan, which previously always defaulted a genuinely-new row to `indexed=true` at the schema level with no way to configure or disable that.
  ...
  - **Follow-up, same day**: the setup wizard's channels step (`/channels`) offers this exact setting up front too, not just the admin page after the fact — a checkbox alongside the channel-selection table (`wizard_channels.html`), persisted via `wizard_queries.set_auto_index_new_channels` (delegates to the same `admin_queries` functions, matching `confirm_channel_selection`'s existing delegation to `set_channel_indexed`). Defaults checked (today's behavior). A real CSS bug was found and fixed along the way: `.wizard-form input { display: block; width: 100%; }` was written for text inputs but also stretched the new checkbox into a full-width box, visually centering its glyph away from its label text — fixed by scoping that rule to `input:not([type="checkbox"])` and giving checkboxes their own small, inline rule. Integration tested (`test_wizard_channels_step.py`); manually verified in a real browser (screenshots) that the checkbox defaults checked, and that unchecking it and submitting persists `false` on a later revisit.
