...
- [ ] **Gap found while scoping the concurrency cap above, not yet built**: there's no orchestrator that discovers a guild's channels and runs `backfill_channel()` across all of them on startup. `reconcile_guild()` (Phase E) only touches channels that already have a `channels` row (i.e. backfill already ran for them); nothing currently creates that first row or performs the initial backfill for a channel Threadbare has never seen. `BoundedHistoryFetcher`/`RetryingHistoryFetcher` are ready for this (they wrap any `HistoryFetcher`), but there's nothing multi-channel yet to actually exercise the concurrency cap — today `reconcile_guild` processes channels strictly sequentially. Needed before v1's backfill story is actually complete end-to-end.
