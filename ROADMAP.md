...
- [ ] Admin option to auto-index newly discovered channels by default (permissions allowing), so mods aren't stuck hand-toggling every new channel as it appears. The live `CHANNEL_CREATE` path now always defaults to `indexed=false` (see above) — this item is about the *other* path: `discovery.discover_channels`'s batch reconnect scan still defaults a genuinely-new row to `indexed=true` at the schema level (migration `0001_initial_schema.sql`), with no way to configure that default, let alone turn it off. Would need a persisted site-wide setting (no settings table exists yet — `web/views/admin.py`'s toggle is per-channel only), a read of that setting from `discover_channels`, and a new admin-page control.
