...
Quality-of-life features that make it a *good* forum rather than a functional one, in rough priority order: unread tracking and "first unread post" jumping (per-user read markers — the single most-missed forum feature); reply-chain threading view as an alternative to flat pagination; user post-history search; emoji/reaction filtering ("show me everything this server 🔥'd"); theme extensibility beyond the four shipped in v1 — admins can register new custom themes (paste/upload CSS, validated against the existing custom-property contract `theme-plain.css` already defines) and download any theme, shipped or custom, as a plain `.css` file. This is the concrete first slice of "community theme contributions": a themes/ directory plus a gallery page can build on it later, still entirely within the CSS-only theme contract that makes this cheap. Estimate: continuous, 0.5–2 days per feature; none blocks anything. (RSS/Atom feeds per board were considered here too but deferred, by explicit user choice, as out of scope for this project — see `ROADMAP.md`'s Phase 3 checklist.)
