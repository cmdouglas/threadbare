...
| Deep pagination and deep search results are slow against a genuinely million-row board — the `<200ms` acceptance criterion (ROADMAP.md) doesn't hold at that extreme | `db/queries.get_messages_page` paginates via `OFFSET`, not a keyset cursor: EXPLAIN ANALYZE against a real 1,000,000-row seeded channel (`tests/performance/test_browse_performance.py`) confirms Postgres walks all ~999,975 preceding index entries to serve the last page (~700-850ms), even though only 25 rows come back. `search_messages`'s per-result `preceding_count` correlated subquery has the same shape, worse: it reruns that walk once per result row (~2.2s for 25 results). Both are measured, real, and left unfixed this pass — a proper fix means switching board/topic (and search-result-context) pagination to keyset cursors project-wide, out of scope for the pass that added the performance test tier. The two affected tests are marked `xfail` (not skipped, not weakened) specifically so this can't silently regress out of view. First-page loads and the board index itself are unaffected (both comfortably under budget once `get_board_post_aggregates` was fixed to stop sorting the whole table just to find one board's latest post — see `RESOLVED_ISSUES.md`). |
