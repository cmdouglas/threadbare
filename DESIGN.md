...
| **(found on a real production deployment) A real image attachment rendered as a plain filename link instead of inline** | `rendering/attachments.py`'s image check relied solely on `attachments.content_type`, but Discord omits that field for some attachments (older uploads, or server-side detection failures — confirmed by reading discord.py's own source: `content_type: Optional[str] = data.get('content_type')`, a `NotRequired` key in Discord's own payload shape). No existing test covered a missing `content_type` paired with an image-extension filename. Fixed with a fallback to `mimetypes.guess_type(filename)` when `content_type` is absent, since `filename` is the one attachment field Discord always sends. |
| **(found on a real production deployment) Animated custom emoji and animated-GIF link unfurls both rendered as a static first frame** | Two independent gaps with the same symptom. (1) `rendering/markdown.py`'s animated-custom-emoji handling (§5 feature 5) stripped the `a:` prefix to work around the parser library's missing animated branch, but then hardcoded `animated=False` when rendering — losing the animated bit entirely instead of just working around the parser. Fixed by scanning the raw content for animated-emoji ids separately from the parse step. (2) `rendering/embeds.py` never captured `embed.video` at all — for a "gifv"-type embed (e.g. Tenor/Giphy), `embed.image`/`embed.thumbnail` are only ever a static preview frame; the real animated content is a wholly separate field this project's `EmbedLike` Protocol didn't model. Fixed by capturing `video.url` through the full pipeline (new `embeds.video_url` column, migration `0007_embed_video.sql`) and rendering a `<video autoplay loop muted playsinline>` tag in place of the static image when present. |
| **(found on a real production deployment, via a side-by-side screenshot against the real Discord client) A bot/webhook message showed a literal `:game_die:` shortcode instead of the real 🎲 emoji** | `rendering/markdown.py`'s `NodeType.EMOJI_UNICODE_ENCODED` handling had always fallen back to literal shortcode text on the stated assumption that "no local emoji-name lookup exists" — true until the `emoji` package (§5 feature 9's emoji-tooltip work) was added for an unrelated reason and turned out to make resolving a shortcode back to unicode trivial (`emoji.emojize`). Only bots/webhooks hit this path at all — Discord's own client always resolves `:name:` shortcodes to real unicode before sending, but a bot can (and, per the reporting screenshot, does) send the literal text; Discord's client still renders that literal text as the real emoji, so this project now does too via `resolve_shortcode_to_unicode`, falling back to literal text only for a genuinely unrecognized name. |
