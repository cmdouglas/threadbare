...
8. **Mod controls** — a minimal admin page for the bot installer: choose which channels are indexed (with a planned option to auto-index newly discovered channels by default, so mods don't have to hand-toggle every new one), trigger a resync or a post-grouping regroup, view sync health. The maintenance actions are queued into a `sync_jobs` table the sync worker polls (`sync_worker/jobs.py`) rather than run on the web request — a resync is a full history re-walk, and the job row is also what carries progress and failure back to the page.
...
10. **Consecutive-post merging** — an admin-level, default-off option to render a run of consecutive messages by one author as a single post, so a burst of short messages occupies one slot in the pager rather than a quarter of a page. Non-destructive: messages stay 1:1 with Discord and each row only records whether it *starts* a post (`messages.starts_group`, computed at ingestion by `db/grouping.py`), so permalinks, per-message reactions, edit markers and single-message deletes all keep working and the toggle is instantly reversible. Pagination counts posts rather than messages, which is why the grouping is computed at ingestion rather than at render time: a page boundary has to land between posts, and a render-time grouping can only merge within a page it has already been given. A reply, a system message, a message carrying attachments, or a silence longer than the mod-set gap (default 7 minutes, Discord's own) all start a new post. Deliberately excluded: reaction-filtered views and the reply-tree view, where a partially-shown or tree-spanning post isn't meaningful; and read markers, which stay counted in messages.
...
- Persist Discord roles and each channel's permission overwrites in Postgres — new tables, since neither is captured anywhere today — kept fresh by the sync worker on `GUILD_ROLE_CREATE`/`UPDATE`/`DELETE` and `CHANNEL_UPDATE`, plus each member's current role-ID list (reusing the `GUILD_MEMBERS` intent and `on_member_update` handler already added for display-name refresh — see `ROADMAP.md`'s UI polish backlog, and the initial bulk-fetch gap it doesn't yet cover). This keeps per-user visibility computation a pure Postgres read, matching the rest of the architecture (§4) rather than a live Discord REST call on every page view.
- Compute effective read permission per channel from that stored data using Discord's resolution order: base @everyone permissions → role allows/denies → category overwrites → channel overwrites, with admin short-circuit. Two narrow cases of this exact algorithm already exist and should be generalized rather than reimplemented a third time: `discord_permissions.py`'s `compute_is_public` (the @everyone-only case, used today to decide global indexability) and `wizard/preflight.py`'s `_apply_overwrite_tier`/`compute_bot_effective_permissions` (the bot-only case, used today for the setup wizard's preflight check).
- Cache the resulting channel-visibility set per user session; invalidate on login refresh and the same role/channel-update events above.
- Search must filter by the requesting user's visibility set — this is the easy-to-forget bypass vector (today's `_SEARCH_WHERE_SQL` filters only on `indexed`, with no per-user clause at all), so it's an explicit test target.
...
Quality-of-life features that make it a *good* forum rather than a functional one, in rough priority order: unread tracking and "first unread post" jumping (per-user read markers — the single most-missed forum feature); reply-chain threading view as an alternative to flat pagination; user post-history search; emoji/reaction filtering ("show me everything this server 🔥'd"); theme extensibility beyond the four shipped in v1 — admins can register new custom themes and download any theme (shipped or custom) for reuse. This shipped deliberately maximalist (MySpace-style, per explicit direction): a theme is an uploaded `.zip` **bundle** — `theme.css` (validated against the custom-property contract `theme-plain.css` defines) plus an optional `assets/` folder of media (background images, sprites, fonts, even audio/video) referenced by relative path — not just a bare CSS file. Bundles are validated on upload (contract properties present, no broken asset paths, media-type allowlist, zip-slip/zip-bomb guards) and stored extracted on a filesystem volume (media doesn't belong in Postgres); the DB holds only metadata. This is the concrete first slice of "community theme contributions": a gallery page can build on it later. Estimate: continuous, 0.5–2 days per feature; none blocks anything. (RSS/Atom feeds per board were considered here too but deferred, by explicit user choice, as out of scope for this project — see `ROADMAP.md`'s Phase 3 checklist.)
...
#### Recorded departures from this contract

Both of the following were taken deliberately, during the 2026-07-25 code-quality audit pass, and are logged here rather than left implicit — the contract above is only meaningful if breaking it is visible.

1. **Migration `0015_drop_dead_columns.sql` breaks rule 1 (additive-only), and therefore rule 2 (rollback-by-redeploy).** It drops `messages.flags` (written as a literal `0` on every insert and re-asserted by `upsert_message`'s `ON CONFLICT`, never selected anywhere) plus `sync_state.heartbeat_at` and `thread_sync_state.heartbeat_at` (zero references outside their own DDL — superseded by `0002_worker_heartbeat.sql`'s singleton row, with `0003` copy-pasting the already-dead column while mirroring `sync_state`'s shape). Because the previous image still writes `flags`, redeploying it against a `0015`-migrated schema fails on insert — rollback across this one migration is not safe. Accepted *only* because no release has ever been tagged, so no operator can be mid-upgrade. Had a version shipped, this would have had to be an expand/contract pair (stop writing in release N, drop in N+1), and that remains the rule for any future column removal.
2. **`DISCORD_TEST_GUILD_ID` → `DISCORD_GUILD_ID` is a hard rename with no fallback, which breaks rule 3** ("new required config ships with either a safe default, or a guided upgrade step"). Nothing about a deployment's own guild is a test, and the old name had leaked into `.env.example`, `docker-compose.yml`, `deploy/cdk/`'s Secrets Manager keys, and the self-hosting docs. There is no fallback read, so an existing install fails to boot until its `.env` line is renamed. The mitigation, and the whole of the "guided upgrade step" here, is that `config.load_settings` detects the situation specifically — old name set, new name absent — and says so by name (`config.py`'s `_RENAMED_GUILD_ID_ENV_VAR`), rather than emitting a bare "`DISCORD_GUILD_ID` is required". Unit-tested both ways (`tests/unit/test_config.py`). Operators upgrading past this are also told in `docs/self-hosting.md` and `README.md`'s Upgrading sections.
...
- Backups: none for mirrored content — Discord is the source of truth, and re-import is the recovery path (restore config, re-run backfill, tolerate a few hours of rebuild for a large server). Only the small Threadbare-native tables are dumped nightly: mod indexing configuration, setup state, and (Phase 3) per-user read markers — kilobytes, retained 7 days. This also strengthens the compliance posture: with no message backups, deleted content doesn't persist anywhere, making deletion honoring unconditional rather than "within backup retention." One exception to the "small tables only" rule arrived with Phase 3's custom themes: their *metadata* is in Postgres (captured by the dump), but the uploaded bundle files (CSS + images/fonts/audio/video) live on a separate `threadbare-themes` filesystem volume the DB dump does **not** capture — a deliberate storage choice (media doesn't belong in Postgres) that makes the themes volume a second, operator-managed backup target. A lost/rebuilt volume degrades gracefully (every viewer falls back to a built-in theme; no broken pages), and themes are re-uploadable, so this is a convenience-loss recovery, not a data-integrity one — but it's called out in `docs/self-hosting.md` so operators back the volume up (or snapshot the VPS) rather than assume the DB dump covers it.
...
Real bugs found and fixed (mostly against a live deployment or a real
Discord server) are logged in [`RESOLVED_ISSUES.md`](./RESOLVED_ISSUES.md)
rather than here, to keep this table to genuinely open risks.
...
| The board index (`/`) occasionally lands right at or just over the `<200ms` acceptance criterion (ROADMAP.md) against a genuinely million-row seeded channel, while every other measured page is comfortably under budget | `tests/performance/test_browse_performance.py::test_board_index_loads_under_200ms` measured 99ms in one clean run and 203-213ms in two others, all against the same fixture with no code path difference — looks like ordinary local-Postgres/dev-machine variance around a razor-thin 200ms budget rather than a real query cost problem (the route's own queries, `get_boards_and_categories`/`get_board_post_aggregates`, weren't touched by the pagination/search fixes below and aren't newly slow). Left as an observed, unexplained flake risk rather than "fixed" with a guess — not reproduced with an EXPLAIN ANALYZE root cause the way the two gaps below were before they were fixed. |
| The `DISCORD_TEST_GUILD_ID` → `DISCORD_GUILD_ID` rename (§7's recorded departure 2) has never been exercised against a real upgrade | Verified by inspection and unit tests only: `config.load_settings` raises a rename-naming `ConfigError` when the old name alone is set, and every in-repo reference was renamed together (`.env.example`, `docker-compose.yml`, `deploy/cdk/lib/web-stack.ts`'s Secrets Manager keys, `docs/self-hosting.md`, `scripts/`, all test fixtures). What is *not* verified: that a real Options A/B upgrade surfaces that error legibly through Compose's logs, and that an Option C operator's `threadbare/app-config` secret — which they must rekey by hand, since CDK reads the key name from the secret — actually fails closed rather than silently starting with a missing guild id. Flagged per this document's own convention rather than assumed. |
...
| The four built-in themes are hand-synced by convention, and they double as the worked example every third-party bundle author copies — so a drift between them propagates into themes we don't control | Three unit tests in `tests/unit/web/test_theme_css.py` carry the invariants that can be checked statically: an identical `:root` property set across all four, an identical numbered section layout (so the four stay readable side by side), and every selector the templates emit styled by every theme. `threadbare/theme_bundle.py` documents the contract as a *minimum* with an explicit optional-property list, and `theme-plain.css` is the prose reference the other three defer to. What no test covers is whether a rule *looks* right — the `.board-row-visited`-versus-zebra-stripe and vBulletin header-nav contrast bugs were both valid CSS that rendered wrong, and both were found by reading, not by a failing test. Visual review of all four themes stays a manual release step (ROADMAP.md's verification checklist). |
